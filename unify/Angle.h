...
namespace unify
{
	constexpr float TAU = 6.28318530f; // 2 * pi, or the number of radians in a circle.
	constexpr float PI = 3.14159265f;
	constexpr float HALF_PI = 1.57079632f;
	constexpr float DTOR = (3.14159265f / 180.0f);
	constexpr float RTOD = (180.0f / 3.14159265f);
	...
	class Angle
    {
		// We cannot create a Angle without knowing if it is in radians or degrees, so we make the 
		// constructors private, and only allow construction through the friend functions (named constructors).
	...
	public:
		...
		/// </definition>
		Angle( const std::string_view angle );
